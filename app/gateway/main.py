...
# TEMPORARY: live verification of the LLM-based modify-intent classification. Remove after use.
...
@app.post("/admin/debug/llm_modify_check")
async def debug_llm_modify_check(request: Request) -> JSONResponse:
    from app.gateway.customer import _llm_confirms_modify_intent
    params = await _parse_body(request)
    return JSONResponse({"confirmed": _llm_confirms_modify_intent(params["text"])})
