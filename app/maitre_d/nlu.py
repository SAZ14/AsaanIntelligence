...
Claude turns free text ("table for 4 this Friday around 8ish, it's Ayesha")
into a structured :class:`ParsedMessage`. A deterministic regex parser is the
fallback whenever no Claude client is supplied or the API call fails — so the
agent (and the test-suite) never hard-depend on the network.

Per the v1 design: *Claude understands, code decides.* Nothing here books,
cancels or holds a table; it only extracts intent + slots.
"""
...
class ParsedMessage:
    ...
    confidence: str = "low"          # "high" (Claude) | "low" (fallback)
    raw: str = ""
...
def parse_message(
    ...
    *,
    ...
    """Parse one inbound message. Uses Claude when available, else fallback."""
    now = now or datetime.now()
    if client is not None:
        parsed = _parse_with_claude(text, now, client)
...
def _parse_with_claude(
    text: str, now: datetime, client: anthropic.Anthropic
) -> ParsedMessage | None:
...
def _extract_time(lower: str) -> tuple[int | None, int | None]:
    ...
    if m:
        h = int(m.group(1))
        if h <= 11:  # café "8ish" means evening
            h += 12
        return h, 0
