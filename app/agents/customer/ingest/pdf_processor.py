...
def process_and_store_pdf(media_url: str, filename: str, store_id: int) -> int:
    """Download a PDF, chunk it, embed it, and store in the knowledge base for store_id."""
    pdf_bytes = download_twilio_media(media_url)
    text = extract_text_from_pdf(pdf_bytes)
    if not text.strip():
        return 0
    chunks = chunk_text(text)
    from app.agents.customer.community.store import store_knowledge_chunks, clear_knowledge_by_source
    clear_knowledge_by_source(store_id, filename)
    documents = [{"content": c, "metadata": {"source": filename}} for c in chunks]
    store_knowledge_chunks(store_id, documents)
    return len(chunks)
