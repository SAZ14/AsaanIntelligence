...
The agent never talks to a payment processor directly -- it goes through a
small PaymentProvider interface so a real gateway (Stripe, a local PSP) can
be dropped in without touching the decision engine. The bundled
StubPaymentProvider issues deterministic links and confirms them via a
plain webhook payload, so the whole deposit flow runs end-to-end without a
real gateway.

KNOWN LIMITATION (carried over from the maitre-d-agent branch, not yet
resolved by this port): no real payment gateway is wired anywhere in this
codebase. Until one is, high-no-show-risk holds get a fake checkout link
and can only be confirmed manually (or via a test call to the payment
webhook) -- deposits do not actually collect money in production yet.
...
Flow:
    ...
    2. the link + a payment_ref are sent to the guest and stored on the
       reservation;
    3. when the guest pays, the gateway calls our payment webhook; we hand
       the payload to parse_webhook -> (payment_ref, paid) and, if paid,
       flip the reservation pending -> confirmed.
"""
